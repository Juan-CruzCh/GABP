...
class Formulario_materiales(forms.ModelForm):
    class Meta:
        model=Materiales
        fields = ['nombre', 'codigo','unidad_manejo', 'categoria']
        widgets = {
...
class Form_infomacion_material(forms.ModelForm):
    class Meta:
        ...
        fields= ['cantidad_paquete_unidad', 'precio_unidad','factura','proveedor' ]
        widgets = {
    ...
    def __init__(self, *args, **kwargs):#mostrando en un select todas las categorias disponibles
        super(Form_infomacion_material,self).__init__(*args, **kwargs)
